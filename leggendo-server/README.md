...
Feature de la formule Premium IA, mais **hors du système de crédits** : c'est un flux partagé généré par le cron, pas une génération à la demande consommant un crédit par abonné. Pool partagé, pas un appel par abonné : chaque exécution produit **au plus un seul texte**, lu ensuite par tous les abonnés Premium IA. Cadence cible : **4 à 7 news par heure, 7 au maximum** — un garde-fou dur (`MAX_PER_HOUR` / `countLastHour` dans news-cron.mjs) bloque toute écriture au-delà de 7 textes sur l'heure glissante, quelle que soit la fraîcheur des articles trouvés. Planifier une exécution toutes les 10 minutes ne veut pas dire un texte toutes les 10 minutes : chaque run n'écrit rien si aucun article assez frais et distinct des 50 derniers n'a été trouvé (voir `isFresh`) ou si le quota horaire est déjà atteint — le volume réel suit la cadence de publication des flux sources, dans la limite du quota.
